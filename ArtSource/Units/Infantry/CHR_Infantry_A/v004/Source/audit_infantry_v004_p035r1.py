"""Read-only geometry, topology, rig, pose and preservation audit for P035R1."""
import argparse
import csv
import hashlib
...
def load_build():
    path = Path(__file__).resolve().with_name("build_infantry_v004_p035r1.py")
    spec = importlib.util.spec_from_file_location("p035r1_build_audit", path)
...
def main():
    ...
    if scene.get("SourceVersion") != "CHR_Infantry_A_v004_P035R1":
        raise RuntimeError("Opened blend is not P035R1")
    ...
    after = build["after"]
    source_hash = sha256(bpy.data.filepath)
    ...
    summary = {
        ...
        "opened_file_sha256": source_hash,
        "build_result_sha_matches": source_hash == build["output_sha256"],
        ...
        "ratios": {
            "upper_arm": after["segment_lengths_normalized"]["upper_arm_length"],
            "forearm": after["segment_lengths_normalized"]["forearm_length"],
            "combined_shoulder_to_wrist": after["segment_lengths_normalized"]["upper_arm_length"] + after["segment_lengths_normalized"]["forearm_length"],
            "hand_width": after["horizontal_measurements_normalized"]["hand_width"],
            "hand_length": after["segment_lengths_normalized"]["hand_length"],
            "head_width": after["horizontal_measurements_normalized"]["head_width"],
        },
        "preserved": {
            "shoulder_width": after["horizontal_measurements_normalized"]["anatomical_shoulder_width"],
            "hip_y": after["vertical_landmarks_normalized"]["L03_Crotch_HipJoint"],
            "knee_y": after["vertical_landmarks_normalized"]["L02_Knee"],
            "torso": after["segment_lengths_normalized"]["torso_length"],
            "shield_width_m": after["weapon_measurements_m"]["shield_width"],
            "shield_height_m": after["weapon_measurements_m"]["shield_height"],
            "sword_length_m": after["weapon_measurements_m"]["sword_projected_overall_length"],
        },
    ...
    print("AEGIS_P035R1_AUDIT_COMPLETE", json.dumps(summary, ensure_ascii=False))
