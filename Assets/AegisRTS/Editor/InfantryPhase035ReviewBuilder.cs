...
{
    /// <summary>Builds isolated P03.5 A-pose/L1-pose review assets and captures.</summary>
    public static class InfantryPhase035ReviewBuilder
    {
        private const string Root = "Assets/AegisRTS/Review/InfantryPhase035";
        private const string AposeFbx = Root + "/SK_Infantry_A_v004_P035_Apose_Review.fbx";
        private const string L1PoseFbx = Root + "/SK_Infantry_A_v004_P035_L1Pose_Review.fbx";
        private const string AposePrefab = Root + "/PF_Unit_Infantry_P035_Review.prefab";
        private const string L1PosePrefab = Root + "/PF_Unit_Infantry_P035_L1Pose_Review.prefab";
        private const string ScenePath = Root + "/SCN_Infantry_P035_Review.unity";
        private const string OutputRelative = "docs/ArtProduction/ReviewPackages/Infantry_Phase03_5_L1_Proportion_Alignment_Review/Screenshots/Unity";
        private const string ManifestRelative = "docs/ArtProduction/ReviewPackages/Infantry_Phase03_5_L1_Proportion_Alignment_Review/Manifests/Unity_Capture_Result.json";
        ...
        public static void BuildAndCapture()
        {
            ...
            if (aposeAsset == null || l1PoseAsset == null)
                throw new FileNotFoundException("One or both P035 review FBXs were not imported.");
            ...
            apose.name = "CHR_Infantry_A_v004_P035_Apose_Review";
            l1pose.name = "CHR_Infantry_A_v004_P035_L1Pose_Review";
            ...
            Validate(aposeBounds, aposeBootGround, "A-pose");
            Validate(l1Bounds, l1BootGround, "L1-pose");
            ...
            Debug.Log($"[P035 Review] READY: A={aposeBounds.size.y:F6}m, L1={l1Bounds.size.y:F6}m, renderers={aposeRenderers.Length}/{l1Renderers.Length}.");
        }
        ...
        private static Dictionary<string, Material> CreateMaterials()
        {
            ...
            {
                string path = $"{Root}/MAT_P035_{pair.Key}.mat";
                Material material = AssetDatabase.LoadAssetAtPath<Material>(path);
                if (material == null)
                {
                    material = new Material(shader) { name = $"MAT_P035_{pair.Key}" };
        ...
        private static void Validate(Bounds bounds, float bootGround, string label)
        {
            if (bounds.size.y < 1.80f || bounds.size.y > 1.86f)
