...
| OI-P0-03 | Infantry initial `CHR_Infantry_A_v003` 的Phase 02決定為`CHANGE REQUESTED`；versioned `CHR_Infantry_A_v003_P02R1` 與initial／L1 comparison已完成，但Revision review仍`PENDING`。 | 3D production reference尚未完成；不得把`READY FOR PHASE02 REVISION REVIEW`解讀為Phase 02 PASS或Phase 03 authorization。 | User + art + technical art：審查 `Infantry_Phase02_PrimaryForms_Revision01_Review/04_Review_Checklist.md`，批准、條件批准或提出focused R2。 |
...
- an approved Infantry L2 production reference（Phase 01 target已批准；v003 initial收到change request；P02R1存在但revision review仍pending）;
