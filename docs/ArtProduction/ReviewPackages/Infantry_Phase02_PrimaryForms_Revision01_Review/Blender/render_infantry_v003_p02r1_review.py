"""Generate Phase 02 Revision 01 evidence from CHR_Infantry_A_v003_P02R1.blend.
...
def arguments():
    ...
    parser.add_argument("--initial-blend", required=True)
    argv = []
...
def main():
    ...
    scene = bpy.context.scene
    if scene.get("SourceVersion") != "CHR_Infantry_A_v003_P02R1":
        raise RuntimeError("Opened Blender file is not the v003 P02R1 candidate")
    ...
    summary = {
        ...
        "topology": topology,
        "initial_candidate": str(Path(args.initial_blend).resolve()),
        "initial_candidate_sha256": sha256(Path(args.initial_blend).resolve()),
        "notes": ["Face normals were recalculated by the deterministic revision script.",
    ...
    clay_views = [
        ("Clay_Front", (0, -1, .06)), ("Clay_Left", (-1, 0, .06)),
        ("Clay_Back", (0, 1, .06)), ("Clay_3Q_Front", (-1, -1, .14)),
        ("Clay_3Q_Back", (1, 1, .14)),
    ]
    ...
    # The required initial/L1 comparison sheets are composed after this pass.
    # Both protected inputs remain read-only and are never imported or saved.
    (comparison_dir / "COMPOSITION_SOURCE.txt").write_text(
        "Comparison sheets are composed from the immutable v003 initial captures, "
        "the approved L1 sheet, and the P02R1 captures generated by this script.\n"
        "Run compose_p02r1_comparisons.ps1 after this Blender render pass.\n",
        encoding="utf-8",
    )
    ...
    (unity_dir / "UNITY_REVIEW_CAPTURE_MANUAL_REQUIRED.txt").write_text(
        "UNITY REVIEW CAPTURE: OPTIONAL / NOT RUN\nP02R1 Blender evidence is complete. The formal runtime Prefab was not modified.\n",
        encoding="utf-8")
    print("AEGIS_P02R1_REVIEW_RENDER_COMPLETE", json.dumps({"images": len(images), "summary": summary}, ensure_ascii=False))
